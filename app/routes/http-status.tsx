...
export const meta: MetaFunction = () => {
  return [
    { title: "HTTP Status Codes - Manon.icu" },
    { name: "description", content: "HTTP状态代码概述" },
  ];
};
    ...
    <div className={"mx-auto min-h-screen max-w-4xl px-4 py-6 sm:px-8"}>
      ...
      {data.map((item, key) => (
        <div className={"mt-8"} key={key}>
          <h2 className={"mb-2 flex gap-4 text-xl"}>
            <p>{key + 1}xx</p>
            <p>{title[key]}</p>
          </h2>
          ...
          {item.map((el) => (
            <Link
              ...
              to={`https://www.abstractapi.com/http-status-codes/${el.key}`}
              key={el.key}
            >
